...
class Profiling
{
    static void Main()
    {
        ...
        // SUM xi
        var sum = new SumOperation(numbers.Select(n => new AbsoluteMember(n)).ToList()).GetResult();
        ...
        var sumSquares = new SumOperation(numbers.Select(n =>
            new PowerOperation(2, new List<Term> { new AbsoluteMember(n) })).ToList()).GetResult();

        // N * mean^2
        var meanSquared = new PowerOperation(2, new List<Term> { new AbsoluteMember(mean) }).GetResult();
        ...
        var stddev = new SquareRootOperation(2, new List<Term> { new AbsoluteMember(variance) }).GetResult();
